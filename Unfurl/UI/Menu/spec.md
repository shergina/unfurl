...
- **Decision** (2026-08-24): a camera-trouble block sits above everything, and only when there is trouble.
  - `SRCameraService.onState` drives two items and a separator, all hidden while the state is `.idle` or `.running`, so the common case sees the menu it always saw. The first is a disabled diagnosis ("Camera Access Denied", "No Camera Available", or the `.failed` description); the second is "Open System Settings", shown only for `.unauthorized`, because a missing device or a failed session has nothing to grant.
  - It outranks the posture and camera groups deliberately: none of them can run until it is resolved, and the status item that shows the trouble opens exactly this menu. The panel placeholder already said the same thing, but it can be closed, unpinned, or never opened; the menu is always one click away.
  - The deep link moved to `SRCameraService.openCameraPrivacySettings` so the placeholder and the menu cannot drift. That URL already changed once (the pre-Ventura pane id stopped working), which is the argument against a second copy.
  - No dots on "Open System Settings", per the ellipsis discipline above: the command completes when the pane shows.
