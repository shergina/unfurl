...
- The camera permission prompt still lands at app launch on a fresh install, not on this flow's pages: the menu-bar camera defaults on and attaches the shared session immediately (the panel defaults unpinned since 2026-08-16, so the menu-bar item carries this alone). That is accepted - the about page's privacy block is on screen at that moment, which is the priming. This page's deferral governs the posture probe only.
