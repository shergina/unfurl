...
- **Related code**: `.spec/app.spec.md`; `Unfurl/Tools/spec.md` (`SRCameraService`); `Unfurl/UI/Menu/` (the menu opened on click); `Unfurl/SRSettings.swift` (`showCameraOnStatusBar`; `defaultStatusItemCameraWidth`, `allowedStatusItemCameraWidthRange`).
...
- **Responsibilities**:
  ...
  - Switch content among camera / icon / unavailable based on the preference, device availability, and the camera state (debounced).
...
- **Decision**: show the horizontal resize cursor via a private CoreGraphics SPI, gated on `USE_UNDOCUMENTED_API`.
  ...
  - Chosen: opt into the private call, shipped in all configurations. The call and its symbol names are gated on the `USE_UNDOCUMENTED_API` Swift flag (defined in `Unfurl-Shared.xcconfig` for every configuration), which is the single switch to compile out all deliberate undocumented-API use; removing the flag drops it everywhere in one move.
...
- The resize cursor depends on a private, undocumented CoreGraphics property; a future macOS may change or remove it, in which case the cursor silently reverts to the arrow (no functional impact, the drag still works). Flipping `USE_UNDOCUMENTED_API` off removes it everywhere.
