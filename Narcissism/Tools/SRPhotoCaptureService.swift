...
class SRPhotoCaptureService: NSObject {
		...
		return try await withCheckedThrowingContinuation { continuation in
			delegate.onFinish = { data, error in
				if let data {
					...
					continuation.resume(throwing: error ?? NSError(domain: "narcissism.photo", code: -3, userInfo: nil))
				}
			}
	...
	fileprivate nonisolated static func savePhotoData(_ data: Data) async throws -> URL {
		...
		let fileURL = directoryURL.appendingPathComponent("Narcissism Camera Photo (\(dateString)).jpg", isDirectory: false)
