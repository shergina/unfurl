...
- **Responsibilities**:
  ...
  - `SRCameraPlaceholerView`: show the app logo plus a per-state message, and an "Open System Settings" action when access is denied. Logo, message, and button center as one block (2026-08-14, was logo-at-center with the text hanging below - bottom-heavy); with the message hidden the block collapses to the bare logo, which keeps the watermark-behind-video role at dead center.
